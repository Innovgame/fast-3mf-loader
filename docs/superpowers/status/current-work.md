...
- 下一步应直接从 `docs/superpowers/plans/2026-03-28-stable-1-0-roadmap.md` 开始，而不是重新定义 `1.0` 范围。

## Next Up

- 优先执行 Phase 1：Claim Alignment，先把 README、README-zh、support matrix、类型和当前公开契约对齐。
- 随后执行 Phase 2：Performance Hardening，继续以 `multipletextures.3mf` 与 `truck.3mf` 为 benchmark 主锚点。
- 再进入 Phase 3 / 4：API 稳定化、发布清单和 `1.0` release readiness。
