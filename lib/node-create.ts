...
export function createVertex(state: StateType, input?: ReturnType<typeof extractVertexData>) {
    if (!input) return;
    const current = state.resources.object[state.current.currentObjectId];
    if (!current) return;
...
export function createBasematerial(state: StateType, input?: ReturnType<typeof extractBasematerialData>) {
    if (!input) return;
    const currentBasematerials = state.resources.basematerials[state.current.currentBasematerialsId];
...
export function createColor(state: StateType, input?: ReturnType<typeof extractColorData>) {
    if (!input) return;
    const currentColorGroup = state.resources.colorgroup[state.current.currentColorGroupId];
...
export function createTexture2dCoord(state: StateType, input?: ReturnType<typeof extractTexture2dCoord>) {
    if (!input) return;
    const current = state.resources.texture2dgroup[state.current.currentTexture2dGroupId];
    if (!current) return;
